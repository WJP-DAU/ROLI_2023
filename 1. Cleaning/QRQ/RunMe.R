...
qrq_stage2 <- compare_scenarios(
  ...
  select(
    ...
    ends_with("stage2"), scores_2022, scores_2023, scores_change, total_counts
  )
...
qrq_final <- compare_stages(
  ...
  select(
    ...
    scores_2022, scores_2023, scores_change, match_stage_a_b, total_counts
  )
...
qrq_scores_analysis <- qrq_final %>%
  ...
  mutate(
    diff_qrq_raw = scores_2023 - scores_final,
  ...
  group_by(variables) %>%
  mutate(
    scores_2023_ranking  = rank(-scores_2023),
    scores_final_ranking = rank(-scores_final),
    diff_ranking = abs(scores_2023_ranking - scores_final_ranking),
  ) %>%
  ungroup() %>%
  mutate(
  ...
  filter(
    country %!in% c("Kuwait", "Montenegro")
  )
...
p_diff_ranking  <- plot_bar(data_rankings, 
                            "diff_ranking_cat",
                            "Rankings differences 2023");p_diff_ranking
