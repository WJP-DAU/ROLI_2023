...
qrq_long_2022 <- read_dta("1. Data/3. Final/qrq_long_2022_country_averages.dta") %>%
  select(country, f_1 = f_1_2022, f_2 = f_2_2022, f_3 = f_3_2022, f_4 = f_4_2022, f_5 = f_5_2022, f_6 = f_6_2022, f_7 = f_7_2022, f_8 = f_8_2022, ROLI = ROLI_2022) %>%
  pivot_longer(cols = !c(country), names_to = "variables", values_to = "long_2022")
...
qrq_benchmark <- qrq_long_2022 %>%
  left_join(qrq_long_2023, by = c("country", "variables")) %>%
  mutate(
    long_change     = long_2023 - long_2022,
...
qrq_scores_2022 <- load_scenario("1. Data/3. Final/qrq_country_averages_2022.dta", "scores_2022")
...
qrq4validation <- qrq_scenarios %>%
  ...
  mutate(
    diff_s1_p       = scores_s1_p - scores_2022,
    ...
    s1_p_match_long = if_else(long_direction == s1_p_direction & total_counts_long > 5, "Same", "Different",
                              if_else(s0_direction == s1_p_direction, "Same", "Different",
                                      "Review")),
    diff_s1_n       = scores_s1_n - scores_2022,
    ...
    s1_n_match_long = if_else(long_direction == s1_n_direction & total_counts_long > 5, "Same", "Different",
                              if_else(s0_direction == s1_n_direction, "Same", "Different",
                                      "Review")),
    diff_s2_p       = scores_s2_p - scores_2022,
    ...
    s2_p_match_long = if_else(long_direction == s2_p_direction & total_counts_long > 5, "Same", "Different",
                              if_else(s0_direction == s2_p_direction, "Same", "Different",
                                      "Review")),
    diff_s2_n       = scores_s2_n - scores_2022,
    ...
    s2_n_match_long = if_else(long_direction == s2_n_direction & total_counts_long > 5, "Same", "Different",
                              if_else(s0_direction == s2_n_direction, "Same", "Different",
                                      "Review")),
    diff_s3_p       = scores_s3_p - scores_2022,
    ...
    s3_p_match_long = if_else(long_direction == s3_p_direction & total_counts_long > 5, "Same", "Different",
                              if_else(s0_direction == s3_p_direction, "Same", "Different",
                                      "Review")),
    diff_s3_n       = scores_s3_n - scores_2022,
    ...
    s3_n_match_long = if_else(long_direction == s3_n_direction & total_counts_long > 5, "Same", "Different",
                              if_else(s0_direction == s3_n_direction, "Same", "Different",
                                      "Review")),
    diff_s4_p       = scores_s4_p - scores_2022,
    ...
    s4_p_match_long = if_else(long_direction == s4_p_direction & total_counts_long > 5, "Same", "Different",
                              if_else(s0_direction == s4_p_direction, "Same", "Different",
                                      "Review")),
    diff_s4_n       = scores_s4_n - scores_2022,
...
qrq_validated <- validate_qrq_scores(qrq4validation) %>%
  select(country, variables, validated_score, validated_scenario, total_counts, 
         scores_2022, scores_2023, scores_change, scores_direction,
...
factors_scores <- qrq_validated %>%
  mutate(
    factor = 
      case_when(
        str_detect(variables, "f_1") ~ "f_1",
        str_detect(variables, "f_2") ~ "f_2",
        str_detect(variables, "f_3") ~ "f_3",
        str_detect(variables, "f_4") ~ "f_4",
        str_detect(variables, "f_6") ~ "f_6",
        str_detect(variables, "f_7") ~ "f_7",
        str_detect(variables, "f_8") ~ "f_8"
      )
  ) %>%
...
qrq4analysis <- qrq_validated %>%
  mutate(
    diff_scores_2022            = validated_score - scores_2022,
    validated_scores_direction  = if_else(diff_scores_2022 > 0, "Positive", "Negative"),
  ...
  mutate(
    ...
    diff_scores_2023_cat = case_when(
      diff_scores_2023 < 0.01     ~ "Differences below 0.01",
      diff_scores_2023 >= 0.01 & diff_scores_2023 < 0.05 ~ "Differences between 0.01 and 0.05",
      diff_scores_2023 >= 0.05                       ~ "Differences above 0.05"
    )
  ) 
...
data_diff       <- proportions_data(subfactors_analysis,
                                    "diff_scores_2023_cat", diff_levels)

p_differences   <- plot_bar(data_diff, 
                            "diff_scores_2023_cat",
                            "Distribution of differences 2023");p_differences
...
p_direction     <- plot_bar(data_direction, 
                            ...
                            "Differences in the direction 2023");p_direction  
